...
describe("Hook Management", () => {
  ...
  describe("replaceVoiceHooks", () => {
    it("should remove old voice hooks and add new ones", () => {
            ...
            hooks: [
              {
                type: "command",
                command:
                  'curl -s -X POST "http://localhost:${MCP_VOICE_HOOKS_PORT:-5111}/api/hooks/old-stop" || echo \'{"decision": "approve"}\'',
              },
            ],
          },
          {
      ...
      expect(result.Stop[0].hooks[0].command).toBe("custom-stop.sh");
      expect(result.Stop[1].hooks[0].command).toBe(
        'curl -s -X POST "http://localhost:${MCP_VOICE_HOOKS_PORT:-5111}/api/hooks/stop" || echo \'{"decision": "approve", "reason": "voice-hooks not running"}\''
      );
    });
  ...
  describe("areHooksEqual", () => {
    ...
    it("should return true for reordered hook types", () => {
      const hooks1: HookSettings = {
        ...
        PostToolUse: [
          { matcher: "", hooks: [{ type: "command", command: "post.sh" }] },
        ],
      };
    ...
    it("should return false for different hooks", () => {
      const hooks1: HookSettings = {
        Stop: [
          { matcher: "", hooks: [{ type: "command", command: "stop1.sh" }] },
        ],
      };

      const hooks2: HookSettings = {
        Stop: [
          { matcher: "", hooks: [{ type: "command", command: "stop2.sh" }] },
        ],
      };
    ...
    it("should return false for different number of hooks", () => {
      const hooks1: HookSettings = {
        Stop: [{ matcher: "A", hooks: [{ type: "command", command: "a.sh" }] }],
      };
